...
public class PackageListAdapter extends BaseAdapter implements Runnable {
    ...
    static class MyInnerHandler extends Handler{
        WeakReference<PackageListAdapter> mPackListAdapt;

        MyInnerHandler(PackageListAdapter adapter) {
            mPackListAdapt = new WeakReference<PackageListAdapter>(adapter);
        }

        @Override
        public void handleMessage(Message message) {
            PackageListAdapter adapter = mPackListAdapt.get();

            PackageItem item = (PackageItem) message.obj;
            int index = Collections.binarySearch(adapter.mInstalledPackages, item);
            if (index < 0) {
                adapter.mInstalledPackages.add(-index - 1, item);
            } else {
                adapter.mInstalledPackages.get(index).activityTitles.addAll(item.activityTitles);
            }
    ...
    public static class PackageItem implements Comparable<PackageItem> {
        ...
        @Override
        public int compareTo(@NonNull PackageItem another) {
    ...
    public void run() {
        ...
        for (ResolveInfo info : installedAppsInfo) {
            ...
            final PackageItem item = new PackageItem(appInfo.packageName, appInfo.loadLabel(mPm), appInfo.loadIcon(mPm));
